...
function hiform_bmp_process() {
  ...
  preStartJulian = preDate1.dayofYear() + 1;
  preEndJulian = preDate2.dayofYear() + 1;

  console.log(
    preDate1.getYear() + 1900,
    preDate2.getYear() + 1900,
    preStartJulian,
    preEndJulian
  );
  ...
  postStartJulian = postDate1.dayofYear() + 1;
  postEndJulian = postDate2.dayofYear() + 1;
  ...
  // Perform clip on larger extent 'bounds' image to geometry or shapefile
  var greenest_withNDVIDATE_gsmax_clip = greenest_withNDVIDATE_gsmax;
  ...
  // Perform clip on larger extent 'bounds' image to geometry or shapefile
  var greenest_withNDVIDATE_winmax_clip = greenest_withNDVIDATE_winmax;
  ...
  //    Perform absolute change NDVI image differencing
  var gsmaxMINUSwinmax = greenest_withNDVIDATE_gsmax_clip.subtract(
    greenest_withNDVIDATE_winmax_clip
  );
  //print(gsmaxMINUSwinmax,'gsmaxMINUSwinmax');

  var ADD_gsmaxMINUSwinmax = greenest_withNDVIDATE_gsmax_clip.add(
    greenest_withNDVIDATE_winmax_clip
  );
  ...
  let ndviRamp = { min: -127, max: 127, palette: ndvi_palette };
  ////////////////////////////////////////////////////////////////////
  ...
  var forest2019 = nlcd_2019_landcover_img
    .eq(41)
    .or(nlcd_2019_landcover_img.eq(42))
    .or(nlcd_2019_landcover_img.eq(43))
    .or(nlcd_2019_landcover_img.eq(90));
  var pineforest2019 = nlcd_2019_landcover_img
    .eq(42)
    .or(nlcd_2019_landcover_img.eq(43));
  var hwdforest2019 = nlcd_2019_landcover_img
    .eq(41)
    .or(nlcd_2019_landcover_img.eq(90));

  // all 4-forest classes
  var nlcd_wild = nlcd_2019_landcover_img
    .eq(41)
    .or(nlcd_2019_landcover_img.eq(42))
    .or(nlcd_2019_landcover_img.eq(43))
    .or(nlcd_2019_landcover_img.eq(90));
  var nlcd_wild_mask = nlcd_wild.selfMask();
  ...
  // Map.addLayer(slope.mask(forest2019), {min: 0, max: 60, palette: [
  ...
  //TODO: EXPORTING Export.image.toDrive({image: absNDVIc_x100sint8.sldStyle(sld_intervals).mask(nlcd4_mST_mask), description: 'S2TOA_2bins_severeANDmoderate_1yrNDVIchange_092122_RGB_pre052121_072121_post072122_092122_v030923',region:geometry,'scale':10, 'maxPixels':1e13});

  ////  Export forest-masked, continuous value raster, requires color map, editable on desktop, threshold/isolate ndvi change values
  //Export.image.toDrive({image: absNDVIc_x100sint8.sldStyle(sld_intervals).mask(nlcd4_mST_mask), description: 'S2SR_Harvests_HarrisCo_GA_1yrNDVIchange_092122_sINT8bit_pre072121_092121_post072122_092122_v030723',region:geometry,'scale':10, 'maxPixels':1e13});
  ...
  //Map.addLayer(absNDVIc_x100sint8.sldStyle(sld_intervals).mask(dec_range_x100sint8), {}, 'dec forest departure by ST', false);
  //Map.addLayer(absNDVIc_x100sint8.sldStyle(sld_intervals).mask(evgrn_range_x100sint8), {}, 'evgrn-mixed forest departure by ST', false);

  // Export hybrid-forest-masked, photo-like RGB composite, pre-colorized
  //Export.image.toDrive({image: absNDVIc_x100sint8.sldStyle(sld_intervals).mask(decforest_mask), description: 'S2TOA_H_Sally_091620_DecForest20162006_dNDVI_RGB_pre071520_091520_post091720_100420_v100620',region:geometry,'scale':10, 'maxPixels':1e13});
  //Export.image.toDrive({image: absNDVIc_x100sint8.sldStyle(sld_intervals).mask(evgrnforest_mask), description: 'S2TOA_H_Sally_091620_EvgrnForest20162006_dNDVI_RGB_pre071520_091520_post091720_100420_v100620',region:geometry,'scale':10, 'maxPixels':1e13});
  ...
  var forchange2 = absNDVIc_x100sint8.mask(nlcd4_mST_mask); // DEFAULT, reports both evergreen and deciduous harvest combined
  // var forchange = absNDVIc_x100sint8.mask(eg_3masks); // for evergreen harvest output
  // var forchange = absNDVIc_x100sint8.mask(dec_3masks); // for deciduous harvest output, to make active remove the //, and comment-out line 876 with // at beginning of line

  //var forchange_tdn = forchange.mask(forchange.select(['NDVI']).lte(-20));
  var forchange_tdn2 = forchange2.mask(
    forchange2
      .select(["NDVI"])
      .lte(-7)
      .and(forchange2.select(["NDVI"]).gte(-19))
  );

  var tholdmask2 = forchange_tdn2
    .select(["NDVI"])
    .lte(-7)
    .and(forchange_tdn2.select(["NDVI"]).gte(-19));
  //print(tholdmask,'tholdmask');
  //Map.addLayer(tholdmask2, {}, 'Forest Thold mask', false);

  // // Compute the number of pixels in each patch.
  // //var patchsize = tholdmask.connectedPixelCount(100, false);
  var patchsize2 = tholdmask2.connectedPixelCount(50, false); // default 100
  ...
  // Map.addLayer(forchange_tdn.mask(), {}, 'Forest Thold change');

  // Get rid of any patches that are less than 36 pixels.
  var patchsizeLarge2 = patchsize2.gte(36); // 40pix = 1ac, 50pix = 0.5 ha
  ...
  var zones2 = forchange_tdn2
    .lte(-7)
    .and(forchange_tdn2.select(["NDVI"]).gte(-19))
    .selfMask()
    .updateMask(patchsizeLarge2);
  ...
  var forchange = absNDVIc_x100sint8.mask(nlcd4_mST_mask); // DEFAULT, reports both evergreen and deciduous harvest combined
  // var forchange = absNDVIc_x100sint8.mask(eg_3masks); // for evergreen harvest output
  // var forchange = absNDVIc_x100sint8.mask(dec_3masks); // for deciduous harvest output, to make active remove the //, and comment-out line 876 with // at beginning of line
  ...
  //print(tholdmask,'tholdmask');
  //Map.addLayer(tholdmask, {}, 'Forest Thold mask', false);
  ...
  // Get rid of any patches that are less than 36 pixels.
  var patchsizeLarge = patchsize.gte(36); // 40pix = 1ac, 50pix = 0.5 ha
  ...
  var zones = forchange_tdn.lte(-20).selfMask().updateMask(patchsizeLarge);
  zones = zones.selfMask();
  //print(zones,'zones');
